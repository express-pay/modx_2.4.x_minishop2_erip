...
class Erip extends msPaymentHandler implements msPaymentInterface
{
    public $modx;
    ...
    public function send(msOrder $order)
    {
        ...
        if (isset($response['Errors'])) {
            ...
            $output = str_replace('##ORDER_ID##', $id,  $output);
            $output = str_replace('##ERIP_PATH##', $this->modx->getOption('EXPRESS_PAY_PATH_TO_ERIP'),  $output);
    ...
    public function getQrCode($ExpressPayInvoiceNo)
    {
        $request_params_for_qr = array(
            "Token" => $this->modx->getOption('EXPRESS_PAY_TOKEN_ERIP'),
            "InvoiceId" => $ExpressPayInvoiceNo,
            'ViewType' => 'base64'
        );
        $request_params_for_qr["Signature"] = $this->compute_signature($request_params_for_qr, $this->modx->getOption('EXPRESS_PAY_SECRET_WORD_ERIP'), 'get_qr_code');
    ...
    public function getInvoiceParam(msOrder $order)
    {
        ...
        $request = array(
            "ServiceId"          => $this->modx->getOption('EXPRESS_PAY_SERVICE_ID_ERIP'),
            "AccountNo"          => $id,
            "Amount"             => $amount,
    ...
    public function compute_signature($request_params, $secret_word, $method = 'add_invoice')
    {
        ...
        $result = $this->modx->getOption('EXPRESS_PAY_TOKEN_ERIP');
